...
In order to also log to a file, just use `logsnake.logfile(..)`:

    logsnake.logfile("/tmp/test.log")

If you want to use specific loggers instead of the global default logger, use
`setup_logger(..)`:

    logger = logsnake.setup_logger(logfile="/tmp/test.log")
...
from logsnake.colors import Fore as ForegroundColors
from logsnake.jsonlogger import JsonFormatter
...
LOGZERO_DEFAULT_LOGGER = "logsnake_default"

# Attribute which all internal loggers carry
LOGZERO_INTERNAL_LOGGER_ATTR = "_is_logsnake_internal"
    ...
    if logfile:
        ...
        if isinstance(_formatter, LogFormatter):
            _formatter = LogFormatter(
                color=False, fmt=_formatter._fmt, datefmt=_formatter.datefmt, colors=_formatter._colors
            )
...
def setup_default_logger(
    ...
    Globally reconfigures the default `logsnake.logger` instance.
...
def reset_default_logger():
    ...
    # Resetup
    logger = setup_logger(
        name=LOGZERO_DEFAULT_LOGGER, logfile=_logfile, level=_loglevel, formatter=_formatter
    )
...
def loglevel(level=DEBUG, update_custom_handlers=False):
    """
    Set the minimum loglevel for the default logger (`logsnake.logger`) and all handlers.
...
def formatter(formatter, update_custom_handlers=False):
    """
    Set the formatter for all handlers of the default logger (``logsnake.logger``).

    This reconfigures only the logsnake internal handlers by default, but you can also
...
def syslog(logger_to_update=logger, facility=SysLogHandler.LOG_USER, disableStderrLogger=True):
    ...
    # Setup logsnake to only use the syslog handler with the specified facility
