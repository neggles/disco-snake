...
async def main():
    ...
    model_provider = SukimaModel(
        "http://192.168.0.147:8000", username="username", password="password", args=model_args
    )
        ...
        try:
            ...
            await ms.add(
                ...
                encoding=array_to_str(
                    await model_provider.hidden_async(model_args, "haru: " + entered, layer=-1)
                ),
            )
            ...
            await ms.add(
                ...
                encoding_model="convo-6B-8bit",
                encoding=array_to_str(
                    await model_provider.hidden_async(
                        model_args, "Patchoulli Knowledge: " + response, layer=-1
                    )
                ),
            )
            ...
            for memory in memories:
                print(
                    f"{memory.created_at} - {memory.author}: {memory.text} - {len(str_to_numpybin(memory.encoding))}"
                )
