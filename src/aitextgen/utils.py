...
def download_file_with_progress(url_base: str, sub_dir: str, model_name: str, file_name: str):
    """
    ...
    r = requests.get(os.path.join(url_base, "models", model_name, file_name), stream=True)
...
def GPT2ConfigCPU(vocab_size: int = 1000, bos_token_id: int = 0, eos_token_id: int = 0, **kwargs):
    """
...
def GPTNeoConfigCPU(vocab_size: int = 1000, bos_token_id: int = 0, eos_token_id: int = 0, **kwargs):
    """
...
def skip_special_tokens(tensor, device, special_token_ids):
    ...
    """
    special_token_id_tensor = torch.unique(torch.as_tensor(special_token_ids)).to(device)
    return tensor[~tensor.unsqueeze(1).eq(special_token_id_tensor.unsqueeze(1)).any(1)].tolist()


def model_max_length(config):
    """Returns the maximum generation length for the given model."""
    return getattr(config, "n_positions", None) or getattr(config, "max_position_embeddings", None)
