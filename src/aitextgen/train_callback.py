...
class ATGProgressCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        if state.is_local_process_zero:
            ...
            avg_loss = 0
            desc = ""
            if self.current_loss == self.current_loss:  # don't add if current_loss is NaN
                avg_loss = self.average_loss(self.current_loss, self.prev_avg_loss, self.smoothing)
    ...
    def generate_sample_text(self):
        ...
        self.training_bar.write(f"\033[1m{self.steps:,} steps reached: generating sample texts.\033[0m")
