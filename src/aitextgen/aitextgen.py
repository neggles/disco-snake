...
class aitextgen:
    ...
    def train(
        ...
        **kwargs,
        ...
        if save_gdrive:
            assert "google.colab" in sys.modules, "You must be in Colaboratory to copy to your Google Drive"
        ...
        if isinstance(train_data, str):
            ...
            logger.info(f"Loading text from {train_data} with generation length of {block_size}.")
        ...
        if tpu_cores > 0 and generate_every > 0:
            logger.info("Generating text during training on a TPU causes issues: disabling generation.")
            generate_every = 0
        ...
        training_args = TrainingArguments(
            ...
            max_steps=num_steps if num_steps else -1,
            ...
            save_strategy=save_strategy,  # we save manually in ATGProgressCallback
    ...
    def train_pt(
        ...
        **kwargs,
        ...
        if os.path.exists(output_dir) and "pytorch_model.bin" in os.listdir(output_dir):
            logger.warning(f"pytorch_model.bin already exists in /{output_dir} and will be overwritten!")
        ...
        if platform.system() == "Windows" and is_gpu_used and n_gpu != 1:
            logger.warning("Windows does not support multi-GPU training. Setting to 1 GPU.")
            n_gpu = 1

        # use the DeepSpeed plugin if installed and specified
        deepspeed_plugin = None
        if is_gpu_used and use_deepspeed:
            deepspeed_plugin = DeepSpeedStrategy()
    ...
    def cross_train(
        ...
        **kwargs,
        ...
        assert len(datasets) == len(learning_rate) == len(num_steps), (
            "The provided learning_rates or num_steps" + " is not equal to the number of inputs."
        )
