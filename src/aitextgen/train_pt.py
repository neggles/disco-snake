...
class ATGProgressBar(ProgressBarBase):
    ...
    def on_batch_end(self, trainer, pl_module):
        ...
        if current_loss == current_loss:  # don't add if current_loss is NaN
            avg_loss = self.average_loss(current_loss, self.prev_avg_loss, self.smoothing)
    ...
    def generate_sample_text(self, trainer, pl_module):
        self.main_progress_bar.write(f"\033[1m{self.steps:,} steps reached: generating sample texts.\033[0m")

        gen_length_max = getattr(pl_module.model.config, "n_positions", None) or getattr(
            pl_module.model.config, "max_position_embeddings", None
        )
