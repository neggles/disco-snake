...
class TokenDataset(Dataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if from_cache:
            ...
            logger.info(f"TokenDataset containing {self.num_subsets:,} subsets loaded {self.str_suffix}")
            return
        ...
        # the text must be processed line-by-line into a a single bulk file
            ...
            assert os.path.isfile(file_path), f"{file_path} is not present in the current directory."
        ...
        if texts:
            self.tokens = encode_tokens_from_list(texts, eos_token, tokenizer, progress_bar_refresh_rate)
        else:
    ...
    def save(self, cache_destination: str = "dataset_cache.tar.gz", compress: bool = True) -> None:
        ...
        if compress:
            ...
            open_func = open
            cache_destination = (
                "dataset_cache.npy" if cache_destination == "dataset_cache.tar.gz" else cache_destination
            )
            compress_str = ""
...
def encode_tokens_from_file(
    ...
    with open(file_path, "r", encoding="utf-8", newline=newline) as f_load:
        ...
        while True:
            if is_csv:
                batch = [text[0] + eos_token for text in list(itertools.islice(f_read, batch_size))]
            else:
                batch = [text + eos_token for text in list(itertools.islice(f_read, batch_size))]
            ...
            for i, encoded_text in enumerate(encoded_texts):
                ...
                tokens[(num_batches * batch_size) + i, : len(encoded_text)] = encoded_text
...
def encode_tokens_from_list(
    ...
    for i_start in range(num_texts // batch_size + 1):
        batch = [
            text + eos_token for text in texts[(i_start * batch_size) : ((i_start * batch_size) + batch_size)]
        ]
