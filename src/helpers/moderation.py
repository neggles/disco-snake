...
if not os.path.isfile("peeps.json"):
    sys.exit("'peeps.json' not found! Please add it and try again.")
else:
    with open("peeps.json") as file:
        peeps = json.load(file)


def open_json():
    with open("peeps.json") as file:
        peeps = json.load(file)
    print(peeps["139971205867372544"][0]["kick_ticket"])


class Chaos(commands.Cog, name="Chaos-slash"):
    ...
    @commands.slash_command(
        ...
        options=[
            Option(
                name="user", description="The user you want to kick.", type=OptionType.user, required=True
            ),
            Option(
                name="reason",
    ...
    async def kick(
        self, interaction: ApplicationCommandInteraction, user: disnake.User, reason: str = "Not specified"
    ) -> None:
        """
        ...
        if member.guild_permissions.administrator:
            print(member, "with user id:", user.id)
            idofuser = int(user.id)
            open_json()
            print(f"{peeps[idofuser][0]['kick_ticket']}")
            # for ticket_count in peeps[user.id]:
            #     print(ticket_count.get("kick_ticket"))
            #                for kick_ticket in ticket_count["kick_ticket"]:
            #                  print(peeps[user.id"kick_ticket"])

            embed = disnake.Embed(title="Error!", description="User has Admin permissions.", color=0xE02B2B)
            ...
            try:
                ...
                await interaction.send(embed=embed)
                try:
                    await member.send(f"You were kicked by **{interaction.author}**!\nReason: {reason}")
    ...
    @checks.not_blacklisted()
    async def nick(
        self, interaction: ApplicationCommandInteraction, user: disnake.User, nickname: str = None
    ) -> None:
        """
    ...
    @commands.slash_command(
        ...
        options=[
            Option(name="user", description="The user you want to ban.", type=OptionType.user, required=True),
            Option(
                name="reason",
    ...
    async def ban(
        ...
        try:
            if member.guild_permissions.administrator:
                embed = disnake.Embed(
                    title="Error!", description="User has Admin permissions.", color=0xE02B2B
                )
                await interaction.send(embed=embed)
            else:
                embed = disnake.Embed(
    ...
    @commands.slash_command(
        ...
        options=[
            Option(
                name="user", description="The user you want to warn.", type=OptionType.user, required=True
            ),
            Option(
                name="reason",
    ...
    async def warn(
        ...
        try:
            ...
            await interaction.send(
                f"{member.mention}, you were warned by **{interaction.author}**!\nReason: {reason}"
            )
    ...
    @checks.not_blacklisted()
    async def hackban(
        self, interaction: ApplicationCommandInteraction, user_id: str, reason: str = "Not specified"
    ) -> None:
        """
